...
// Definisikan helper function di luar route closure untuk menghindari redeclaration error.
if (!function_exists('faqAnswer')) {
...
Route::match(['get', 'post'], '/botman', function (Request $request) {
    ...
    $botman->hears('faq_register', fn($bot) => faqAnswer($bot, 'Bagaimana cara mendaftar?', 'Klik tombol "Daftar" di beranda dan isi nama lengkap, email, dan kata sandi Anda.'));
    $botman->hears('faq_forgot', fn($bot) => faqAnswer($bot, 'Lupa kata sandi akun', 'Klik "Lupa Kata Sandi?" di halaman login dan ikuti instruksi reset.'));
    $botman->hears('faq_book', fn($bot) => faqAnswer($bot, 'Bagaimana cara memesan layanan?', 'Pilih layanan > Pesan > Pilih tanggal > Lanjutkan ke pembayaran.'));
    $botman->hears('faq_payment', fn($bot) => faqAnswer($bot, 'Apa saja metode pembayaran?', 'Kami menerima BCA, Dana, ShopeePay, QRIS, dan opsi pembayaran digital lainnya.'));
    $botman->hears('faq_history', fn($bot) => faqAnswer($bot, 'Bagaimana cara melihat riwayat pesanan?', 'Masuk ke akun Anda dan buka menu "Riwayat Pesanan".'));
...
// RUTE LAIN-LAIN
//======================================================================
// Rute ini akan mengarahkan ke halaman sukses/gagal setelah pembayaran dari Midtrans.
...
// PENTING: Rute untuk notifikasi dari Midtrans HARUS berada di routes/api.php
// dan dikecualikan dari CSRF. Jangan letakkan di sini.
